...
const donateLoading = document.getElementById('donate-loading');
...
function initPaymentSystem() {
    ...
    frequencyOptions.forEach(option => {
        option.addEventListener('click', handleFrequencyChange);
    });
...
function handleCustomAmountInput() {
    ...
    if (amount && amount >= PAYSTACK_CONFIG.donation.minAmount && amount <= PAYSTACK_CONFIG.donation.maxAmount) {
...
function updatePaymentSummary() {
    if (selectedAmount > 0) {
        ...
        summaryAmount.textContent = paystack.formatAmount(selectedAmount);
        summaryFrequency.textContent = selectedFrequency === 'one-time' ? 'One-time' : 'Monthly';
        summaryTotal.textContent = paystack.formatAmount(selectedAmount);
    } else {
        hidePaymentSummary();
    }
}
...
async function handleDonationSubmit(e) {
    ...
    const paymentData = {
        ...
        donorCountry: formData.get('donorCountry'),
        donorMessage: formData.get('donorMessage')
    };
    
    // Validate payment data
    const validation = paystack.validatePaymentData(paymentData);
    ...
    // Show loading state
    setDonateButtonLoading(true);
    
    try {
        // Initialize Paystack payment
        const initResult = await paystack.initializePayment(paymentData);
        
        if (initResult.success) {
            // Redirect to Paystack payment page
            showNotification(PAYSTACK_CONFIG.ui.messages.paymentProcessing, 'info');
            
            // Store payment data in session storage for retrieval after payment
            sessionStorage.setItem('paymentData', JSON.stringify(paymentData));
            sessionStorage.setItem('paymentReference', initResult.reference);
            
            // Redirect to Paystack payment page
            setTimeout(() => {
                paystack.redirectToPayment(initResult.authorizationUrl);
            }, 1000);
        } else {
        ...
        showNotification(PAYSTACK_CONFIG.ui.messages.paymentError, 'error');
        setDonateButtonLoading(false);
    }
}

// Set donate button loading state
function setDonateButtonLoading(loading) {
    if (loading) {
        donateBtn.disabled = true;
        donateBtnText.textContent = PAYSTACK_CONFIG.ui.buttonText.processing;
        donateLoading.style.display = 'inline-block';
    } else {
        donateBtn.disabled = false;
        donateBtnText.textContent = PAYSTACK_CONFIG.ui.buttonText.default;
...
async function verifyPaymentTransaction(reference) {
    try {
        const verifyResult = await paystack.verifyPayment(reference);
        
        if (verifyResult.success) {
            const status = paystack.getPaymentStatus(verifyResult.status);
            
            if (status.status === 'success') {
                showNotification(PAYSTACK_CONFIG.ui.messages.paymentSuccess, 'success');
                ...
                localStorage.setItem('lastDonation', JSON.stringify({
                    ...
                    paidAt: verifyResult.paid_at,
                    gatewayResponse: verifyResult.gateway_response
                }));
                
            } else if (status.status === 'failed') {
                showNotification('Payment was declined. Please try again.', 'error');
                clearPaymentData();
